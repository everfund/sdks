...
describe('DonationForm', () => {
  ...
  it('should render with correct src', () => {
    const { container } = render(
      <DonationForm code={code} />
    );

    const iframe = container.querySelector('iframe');

    expect(iframe).not.toBeNull();
    expect(iframe?.src).toBe(`https://evr.fund/${code}/widget?embed_origin=http://localhost:3000`);
  });
  ...
  it('should render with correct src with custom domain', () => {
    ...
    expect(iframe).not.toBeNull();
    expect(iframe?.src).toBe(`https://new.domain/${code}/widget?embed_origin=http://localhost:3000`);
  });
